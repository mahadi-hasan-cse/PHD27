...
import { ArrowUpDown, BookOpen, CalendarDays, Download, ExternalLink, Search, SlidersHorizontal, Users, X } from 'lucide-react'
...
export default function Home() {
  ...
  function exportCsv() {
    const headers = ['Rank','University','Program','City','State','Fall 2027 deadline (projected)','Application fee USD','Fee waiver / free option','GRE','IELTS Academic','Funding','Official reference','Notes']
    const content = [headers, ...rows.map((u) => [u.rank,u.university,u.program,u.city,u.state,u.deadline,u.fee,u.feeWaiver,u.gre,u.ielts,u.funding,u.programUrl,u.note])].map((row) => row.map(csvCell).join(',')).join('\n')
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="border-b border-border bg-[#071b34] text-white"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4 lg:px-8">
      ...
      <div className="flex items-center gap-2"><a href="/faculty" className={buttonVariants({variant:'outline',className:'border-white/30 bg-transparent text-white hover:bg-white/10 hover:text-white'})}><Users size={16}/> Faculty</a><Button onClick={exportCsv} className="bg-white text-[#071b34] hover:bg-[#e9f2f8]"><Download size={16}/> Export CSV</Button></div>
    </div></header>
